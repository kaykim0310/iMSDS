...
st.set_page_config(page_title="MSDS 섹션 2 - 유해성·위험성", layout="wide", initial_sidebar_state="collapsed")
...
가_내용 = st.text_area("유해성·위험성 분류", value=st.session_state.section2_data.get('가_유해성_위험성_분류', ''), height=150, placeholder="예: 인화성 액체 구분 3\n피부 자극성 구분 2\n심한 눈 손상/자극성 구분 2", key="hazard_classification", label_visibility="collapsed")
...
st.write("**그림문자**")
그림문자 = st.text_input("그림문자", value=st.session_state.section2_data.get('그림문자', ''), placeholder="예: 화염, 느낌표", key="pictogram", label_visibility="collapsed")
st.write("**신호어**")
신호어 = st.text_input("신호어", value=st.session_state.section2_data.get('신호어', ''), placeholder="예: 경고, 위험", key="signal_word", label_visibility="collapsed")
st.write("**유해·위험 문구**")
유해위험문구 = st.text_area("유해·위험 문구", value=st.session_state.section2_data.get('유해위험문구', ''), height=100, placeholder="예: H226 인화성 액체 및 증기\nH315 피부에 자극을 일으킴", key="hazard_statements", label_visibility="collapsed")
st.write("**예방조치문구**")
예방조치문구 = st.text_area("예방조치문구", value=st.session_state.section2_data.get('예방조치문구', ''), height=150, placeholder="예: P210 열·스파크·화염·고온으로부터 멀리하시오\nP280 보호장갑/보호의/보안경/안면보호구를 착용하시오", key="precautionary_statements", label_visibility="collapsed")
...
다_내용 = st.text_area("기타 유해성·위험성", value=st.session_state.section2_data.get('다_유해성_위험성_분류기준에_포함되지_않는_기타_유해성_위험성', ''), height=100, placeholder="예: 자료없음", key="other_hazards", label_visibility="collapsed")
...
with st.expander("저장된 데이터 확인"):
    st.write("### 2. 유해성·위험성")
    항목들 = [("가. 유해성·위험성 분류", '가_유해성_위험성_분류'), ("나. 예방조치문구를 포함한 경고표지 항목", '나_예방조치문구를_포함한_경고표지_항목'), ("다. 유해성·위험성 분류기준에 포함되지 않는 기타 유해성·위험성", '다_유해성_위험성_분류기준에_포함되지_않는_기타_유해성_위험성')]
    for 제목, 키 in 항목들:
